...
public class JiraFetchService {
    ...
    private String extractAdfText(Object node) {
        ...
        if (node instanceof Map) {
            ...
            if ("text".equals(map.get("type"))) return safe(map.get("text"));
            Object content = map.get("content");
            if (content instanceof List) return joinAdfList((List<Object>) content);
        }
        if (node instanceof List) return joinAdfList((List<Object>) node);
        return "";
    }
    ...
    private String extractComments(Object obj) {
        ...
        StringBuilder sb = new StringBuilder();
        int count = 0;
        for (Object c : (List<Object>) commentsList) {
            if (count >= 5) break;
            if (c instanceof Map) {
